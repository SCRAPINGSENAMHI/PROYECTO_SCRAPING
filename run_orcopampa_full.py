...
def try_download(row, from_date, to_date):
    # Priorizar combinaciones probables: remoto mostró ico='M' para ORCOPAMPA
    candidates = []
    if pd.notna(row.get('cod')):
        cod = str(row.get('cod'))
        # prefer remote ico, then CONVENCIONAL/AUTOMATICA, then None
...
def main():
    ...
    if found.empty:
        print('No se encontró ORCOPAMPA en la lista remota')
        return
    ...
    if df is None:
        print('✗ No se encontraron tablas para ORCOPAMPA en el periodo solicitado.')
        marker = pd.DataFrame([{
        ...
        outpath = out_dir / f"ORCOPAMPA_no_data_remote_{to_date}.xlsx"
        marker.to_excel(outpath, index=False)
        print('Marcador guardado en:', outpath)
        return

    outpath = out_dir / f"ORCOPAMPA_{from_date}_to_{to_date}_cod{used_cod}_ico{used_ico}.xlsx"
