...
def main():
    ...
    if found.empty:
        print('No se encontró PAUCARBAMBA en la lista remota')
        return
    ...
    if df is None:
        print('✗ No se encontraron tablas para PAUCARBAMBA en el periodo solicitado.')
        marker = pd.DataFrame([{
        ...
        outpath = out_dir / f"PAUCARBAMBA_no_data_remote_{to_date}.xlsx"
        marker.to_excel(outpath, index=False)
        print('Marcador guardado en:', outpath)
        return

    outpath = out_dir / f"PAUCARBAMBA_{from_date}_to_{to_date}_cod{used_cod}_ico{used_ico}.xlsx"
    try:
